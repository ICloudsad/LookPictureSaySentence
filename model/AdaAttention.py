...
class AdaAttention(nn.Module):
    ...
    def forward(self,V,s_t,h_t):
        ...
        z_t = self.W_h(self.tanh((self.W_v(V).view((64,49,49))+self.W_g(h_t).unsqueeze(1).expand((64,49,49))).contiguous().view(-1,49))).view((64,49)) # 64*49
        beta_t = self.W_h(self.tanh(self.W_s(s_t)+self.W_g(h_t))) # 64*1
        z_t_hat = torch.cat([z_t,beta_t],dim=1)
        alpha_hat = self.softmax(z_t_hat).unsqueeze(2).expand(64,50,512)
        V_s = torch.cat([V.view(64,49,512),s_t.unsqueeze(1)],dim=1) # 64*50*512
